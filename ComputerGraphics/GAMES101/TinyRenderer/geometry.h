...
vec<LEN, T> embed(const vec<DIM, T> &v, T fill = 1) {
    // 向量嵌入，把一个短的向量嵌入一个长向量中，长向量多余部分置为1
    vec<LEN, T> ret;
...
template<size_t LEN, size_t DIM, typename T>
vec<LEN, T> project(const vec<DIM, T> &v) {
    // 向量投影，把一个长向量投影到一个短向量上
    vec<LEN, T> ret;
...
public:
    ...
    mat<DimRows, DimCols, T> adjoint() const {
        // 伴随矩阵
        mat<DimRows, DimCols, T> ret;
        for (size_t i = DimRows; i--;)
            for (size_t j = DimCols; j--; ret[i][j] = cofactor(j, i));
        return ret;
    }

    mat<DimRows, DimCols, T> inverse() const {
        assert(DimRows == DimCols);
        mat<DimRows, DimCols, T> ret = adjoint();
        T tmp = rows[0] * ret.col(0);
        return ret / tmp;
    }
    ...
    mat<DimCols, DimRows, T> transpose() const {
        ...
        for (size_t i = DimCols; i--;)
            for (size_t j = DimRows; j--; ret[i][j] = rows[j][i]);
        return ret;
    };
};
...
template<size_t R1, size_t R2, size_t C1, size_t C2, typename T>
mat<R1, C2, T> operator*(const mat<R1, R2, T> &lhs, const mat<C1, C2, T> &rhs) {
    mat<R1, C2, T> ret;
    for (size_t i = R1; i--;)
