...
{
    // angle：旋转角度，此处默认绕y轴旋转
    // ratio：缩放比例，此处默认等比例缩放
    // position：平移位置
...
## View Trans
```C++
Matrix view_trans(Vec3f e, Vec3f g, Vec3f t){
    // e: 相机位置
    // g: 观测方向，从模型到相机位置
    // t: 视点正上方向
...
Matrix projection_trans(float eye_fov, float aspect_ratio, float zNear, float zFar){
    ...
    // zNear：近平面
    // zFar：远平面
    // 由远平面投射到近平面，从视锥压缩到长方体，在正投射到标准立方体
...
## ViewProt Trans
```C++
Matrix viewport_trans(int w, int h, int d){
    // w:屏幕宽
    // h：屏幕高
    // d：屏幕深度
...
//     实例化模型
    ...
    // Drawing
    Matrix View = view_trans(e,c,t);
    Matrix ViewPort = viewport_trans(width, height, depth);
    Matrix Modeltran =  model_trans(0,Vec3f(1,1,1),Vec3f(0,0,0));
    Matrix Projection = projection_trans(52,1,-0.1,-40);
    // 遍历模型的三角形面片
    for (int i = 0; i < model->nfaces(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            // MVP+ViewPort Trans
            screen_coords[j] = m2v(ViewPort*Projection*View*Modeltran*v2m(world_coords[j]));
...
## Refactor Code
由于main函数过于冗余，并且我们需要对读取 Model 中的法向量等参数，需要统一使用齐次坐标进行变换，因此需要对之前的代码进行一个调整与重新编写。
